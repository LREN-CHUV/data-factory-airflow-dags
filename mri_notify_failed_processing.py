...
post_on_slack = SlackAPIPostOperator(
    ...
    text=':boom: Failed processing for scan session *{{ dag_run.conf["session_id"] }}* performed on {{ dag_run.conf["scan_date"] | default("?") }} for participant {{ dag_run.conf["participant_id"] | default("?") }} at stage {{ dag_run.conf["task_id"] | default("?") }}\n\n'
