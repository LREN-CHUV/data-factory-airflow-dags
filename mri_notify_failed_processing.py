...
post_on_slack = SlackAPIPostOperator(
    ...
    text=':boom: *{{ dag_run.conf["dataset"] }}*: Failed processing for scan session *{{ dag_run.conf["session_id"] }}* {% if dag_run.conf["scan_date"] %}performed on {{ dag_run.conf["scan_date"].strftime("%Y-%m-%d") }}{% endif %} for participant {{ dag_run.conf["participant_id"] | default("?") }}{% if dag_run.conf["task_id"] %} at stage {{ dag_run.conf["task_id"] }}{% endif %}\n\n'
