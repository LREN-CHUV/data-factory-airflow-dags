...
def save_nifti_meta(
    ...
    if not connection.db_session.query(connection.Nifti).filter_by(path=file_path).first():
        ...
        scan = connection.db_session.query(connection.Scan).filter_by(date=scan_date, participant_id=participant_id)\
            .first()
        sequence_type_list = connection.db_session.query(connection.SequenceType).filter_by(name=sequence).all()

        if scan and len(sequence_type_list) > 0:
            scan_id = scan.id
            sess = connection.db_session.query(connection.Session).filter_by(scan_id=scan_id, value=session).first()
